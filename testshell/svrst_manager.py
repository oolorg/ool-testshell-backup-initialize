...
# Fuel --------
	...
	def setup_fuelserver(self, data):
		...
		if ret == -1:
			self.svbkm.br_log(node_id, CLSTER_NAME, br_mode, '#### setup_fuelserver ng file is nothing ')
			return ['NG', 'Error']
		...
		for node_info in data:
			if (node_info['server_type'] == FUEL_SERVER) or \
				(node_info['server_type'] == FUEL6_SERVER) or \
				(node_info['server_type'] == FUEL_AGENT):
				tmp_server_info = {}
				...
				if (tmp_server_info['ip_address'] == -1) or (tmp_server_info['ip_address_c'] == -1):
					self.svbkm.br_log(node_id, CLSTER_NAME, br_mode, '#### setup_fuelserver get nic info Error')
					return ['NG', 'Error']

				if node_info['server_type'] == FUEL_SERVER:
					fuelserver_num += 1
					tmp_server_info['img_name'] = FUEL_IMG
					server_info.append(tmp_server_info)
				elif node_info['server_type'] == FUEL6_SERVER:
					fuelserver_num += 1
					tmp_server_info['img_name'] = FUEL6_IMG
					server_info.append(tmp_server_info)
				else:
					agent_num += 1
					tmp_server_info['img_name'] = UBUNTU12_IMG
					agent_info.append(tmp_server_info)
		...
		ret = fuel_utls.clonezilla_exec(
			self.Token, fuel_utls.MODE_RESTORE, clonezilla_info, agent_info, agent_num)
		if ret == -1:
			self.svbkm.b_log(node_id, CLSTER_NAME, br_mode, '#### fuel-agent clonezilla restore error.')
			return ['NG', 'Error']
		...
		if ret == -1:
			self.svbkm.br_log(node_id, CLSTER_NAME, br_mode, '#### fuel-server clonezilla restore error.')
			return ['NG', 'Error']

		# rmdb update(Fuel Server info)
		self.ori.set_auth(self.Token)
		ret = self.ori.mod_node_data(
			server_info[0]['hostname'], {"os_name": "CentOS", "os_version": "6.5"})
		if -1 == ret[0]:
			self.svbkm.br_log(node_id, CLSTER_NAME, br_mode, '#### mod_node_data err ret=%s' % (ret))

		# Init SW
		ret = self.reset_switch(self.Token, self.node_list)
		if ret[0] == 'NG':
			self.svbkm.br_log(node_id, CLSTER_NAME, br_mode, '#### rest switch Error')
			return ['NG', 'Error']
	...
	def restore_osimage(self, data):
		...
		if ret == -1:
			self.svbkm.b_log("", self.topology_name, '#### restore_osimage clonezilla restore error.')
			return ['NG', 'Error']
		...
		for server in server_info:
			...
			self.ori.set_auth(self.Token)
			ret = self.ori.mod_node_data(
				server_info[0]['hostname'], {"os_name": "ubuntu", "os_version": "12.04"})
			if -1 == ret[0]:
				self.svbkm.b_log(node_id, CLSTER_NAME, br_mode, '#### mod_node_data err ret=%s' % (ret))

			ret = self.set_os_environment(
		...
		code, msg = fuel_utls.http_request(url, param, 'PUT')
		if code != 200:
			self.svbkm.b_log("", self.topology_name, '####restore_osimage mac_mapping NG msg=%s' % msg)
	...
	def reset_postinit(self, **kwargs):
		try:
			br_mode = "r"
			node_id = INIT_NODE
			CLSTER_NAME = self.topology_name
	...
	def init_oo_env(self, data):
		...
		for node_info in data:
			...
			ret = self.wait_os_startup(hostname, username, password)
			if ret == -1:
	...
	def init_ubuntu_env(self, data):
		...
		for node_info in data:
			...
			try:
				self.wait_os_startup(hostname, username, password)
			except:
				self.svbkm.br_log(node_id, CLSTER_NAME, br_mode, '#### init_ubuntu_env wait timeout')
	...
	def init_fuelenv(self, data):
		...
		for node_info in data:
			if node_info['server_type'] == FUEL_AGENT:
				...
				try:
					ret = self.fuel_agent_reboot(hostname, username, password)
				except:
					self.svbkm.br_log(
						node_id, CLSTER_NAME, br_mode, '#### already startup fuel-agent for NCS reboot.')

				# server reboot
				if ret != 0:
					self.svbkm.br_log(
						node_id, CLSTER_NAME, br_mode, 'Any fuel-agent reboot error.')
					return ['NG', 'Error']

		self.svbkm.br_log(node_id, CLSTER_NAME, br_mode, '#### init_fuelenv End')
		return ['OK', 'Success']

	@retry(pexpect.EOF, tries=6, delay=30)
	def fuel_agent_reboot(self, hostname, username, password):

		self.svbkm.b_log(hostname, self.topology_name, '#### fuel_agent_reboot Start')
		...
		if (server_mip == -1):
			self.svbkm.b_log(hostname, self.topology_name, '#### fuel_agent_reboot get M_PLANE Error')
			return -1

		# --- Set Public network env
		try:
			s = pxssh.pxssh()
			s.login(server_mip, username, password, login_timeout=1*60)
			s.sendline('sudo reboot')
			s.expect('.*password for .*', timeout=1*60)
			s.sendline(password)
			s.prompt()
			s.logout()
		...
		self.svbkm.b_log(hostname, self.topology_name, '#### fuel_agent_reboot End')
		return 0

	#@retry(pexpect.EOF, tries=100, delay=10)
	def set_os_environment(self, hostname, username, password):

		self.svbkm.b_log(hostname, self.topology_name, '#### set_os_environment Start')
		...
		server_mip = nicinfo.get_ip_address(nicinfo.M_PLANE)
		if (server_mip == -1):
			self.svbkm.b_log(hostname, self.topology_name, '#### set_os_environment get M_PLANE Error')
			return -1
		...
		self.svbkm.b_log(hostname, self.topology_name, '#### set_os_environment End')
		return 0

	@retry(pexpect.EOF, tries=20, delay=60)
	def wait_os_startup(self, hostname, username, password):
		...
		try:
			s = pxssh.pxssh()
			s.login(server_mip, username, password, login_timeout=2*60)
			s.logout()
	...
	def disable_fuelserver_dhcp(self, hostname, username, password):
		...
		self.svbkm.b_log(hostname, self.topology_name, '#### disable_fuelserver_dhcp Start')
		...
		server_mip = nicinfo.get_ip_address(nicinfo.M_PLANE)
		if (server_mip == -1):
			self.svbkm.b_log(hostname, self.topology_name, '#### disable_fuelserver_dhcp get M_PLANE Error')
			return -1
		...
		self.svbkm.b_log(hostname, self.topology_name, '#### disable_fuelserver_dhcp End')
		return 0
